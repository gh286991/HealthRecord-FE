...
import { WorkoutRecord, WorkoutExercise, useCreateWorkoutMutation, useGetWorkoutListQuery, useUpdateWorkoutMutation } from '@/lib/workoutApi';
...
function WorkoutForm({ initialData, onCancel, onSubmit }: {
  ...
  const [exercises, setExercises] = useState<WorkoutExercise[]>(initialData?.exercises || [
    { exerciseName: 'Bench Press', sets: [{ weight: 40, reps: 10 }] },
  ]);
  const [notes, setNotes] = useState(initialData?.notes || '');

  const addExercise = () => {
    setExercises((prev) => [...prev, { exerciseName: '', sets: [{ weight: 0, reps: 0 }] }]);
  };
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={addExercise} className="inline-flex items-center px-3 py-1 border border-transparent text-sm font-medium rounded-md text-green-700 bg-green-100 hover:bg-green-200">
                  ...
                  {ex.sets.map((s, sIdx) => (
                    <div key={sIdx} className="grid grid-cols-3 gap-3">
                      ...
                      <div className="flex items-end gap-2">
                        <button type="button" onClick={() => removeSet(idx, sIdx)} className="text-red-600 hover:text-red-800">
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                          </svg>
                        </button>
                        <button type="button" onClick={() => addSet(idx)} className="inline-flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-green-700 bg-green-100 hover:bg-green-200">
                          <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                          </svg>
                          新增組次
        ...
        <div className="flex gap-4 pt-6">
          <button onClick={onCancel} className="flex-1 py-3 px-4 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors">取消</button>
          <button onClick={() => onSubmit({ date, exercises, notes })} className="flex-1 py-3 px-4 bg-gradient-to-r from-green-500 to-blue-600 text-white font-medium rounded-lg hover:from-green-600 hover:to-blue-700 transition-all duration-200">{initialData ? '儲存變更' : '新增紀錄'}</button>
        </div>
      </div>
    </div>
  );
